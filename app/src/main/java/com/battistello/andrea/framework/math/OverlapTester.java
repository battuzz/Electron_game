...
public class OverlapTester {
    ...
    public static boolean overlap(Shape s1, Shape s2) {
        if (s1 instanceof Circle) {
            if (s2 instanceof Circle) {
                return overlapCircles((Circle) s1, (Circle) s2);
            }
            else if (s2 instanceof Rectangle) {
                return overlapCircleRectangle((Circle) s1, (Rectangle) s2);
            }
        }
        else if (s1 instanceof Rectangle) {
            if (s2 instanceof Circle) {
                return overlapCircleRectangle((Circle) s2, (Rectangle) s1);
            }
            else if (s2 instanceof Rectangle) {
                return overlapRectangles((Rectangle) s1, (Rectangle) s2);
            }
        }
        return false;
    }
